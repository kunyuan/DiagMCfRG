        ...
        for idx in range(1, Order):
            for i in range(len(Deformation)):
                for j in range(1, idx):
                    Deformation.append(swap_interaction(
...
def Group(PermutationDict, TimeRotation=True):
    ...
    while len(PermutationDict) > 0:
        ...
        Deformation = check_Unique_Permutation(
...
def GenerateAllDiagram(UnlabeledDiagram, InteractionPairs, DoesCheck=True):
    ...
    for d in UnlabeledDiagram:
        ...
        for idx in range(1, Order):
            for i in range(len(Deformation)):
                Deformation.append(swap_LR_Hugen(
...
def SymmetrizeLoops(OptDiagrams, DiagDict, DiagInvDict):
    ...
    for diag in OptDiagrams.keys():
        ...
        print "{0} equals to {1}".format(
            diag, [EqualDict[k][0] for k in EqualDict.keys()])
    ...
    with open("./Diagram/HugenDiag{0}.txt".format(Order)) as f:
        ...
        for d, s in zip(DiagList, Sym):
            ...
            print "{0} with SymFactor: {1}, and Number: {2} with duplicate {3}".format(
                d, s, len(AllDiagrams), len(AllDiagrams)/len(set(AllDiagrams)))
        ...
        print "Total Free energy diagrams: {0}, TotalSym: {1}".format(
            len(FreeEnergyDiagramDict), TotalSym)

        DiagDict, OptDiagDict = GetAllPermutations(
            Order, FreeEnergyDiagramDict, FreeEnergyDiagramInvDict, FreeEnergyDiagramSymDict)
    ...
    for d, s, m in zip(DiagList, SymList, MomList):
        ...
        for g in UnlabeledDiagramList:
            ...
            print "Total {0} with symmetry factor {1}\n".format(
                len(g), float(1.0)/LessPolarDict[g[0]][1])
            ...
            if len(AllDiagrams) == 0:
                ...
                continue
            TotalDiagNum += 1
            UniqueDiagrams.append(
                (g[0], float(len(g))/LessPolarDict[g[0]][1], LessPolarDict[g[0]][0], AllDiagrams))
            PolarUniqueDiagrams.append(
                (g[0], float(len(g))/LessPolarDict[g[0]][1], LessPolarDict[g[0]][0], AllDiagrams))
            ...
            TotalSym += len(AllDiagrams) * \
                abs(float(len(g))/LessPolarDict[g[0]][1])
            AllDiagrams = GenerateAllDiagram(
                AllDiagrams, InteractionPairs, False)
