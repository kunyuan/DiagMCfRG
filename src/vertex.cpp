...
double PhyGreen(double Tau, const mom& Mom);

double Green(double Tau, const mom& Mom, spin Spin, int GType){
    if(GType==0)
...
double Interaction(double Tau, const mom& Mom, spin Spin, int VerType){
    if(VerType<0) ABORT("VerType can not be "<<VerType);
